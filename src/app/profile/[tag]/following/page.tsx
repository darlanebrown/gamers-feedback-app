...
export default function FollowingPage({ params }: { params: { tag: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/profile/${encodeURIComponent(tag)}/following`)
      .then((r) => r.json())
      .then((d) => {
        setTags(d.following ?? []);
        setTotal(d.total ?? 0);
  ...
  return (
    <main className={styles.page}>
      ...
      <header className={styles.header}>
        ...
        {!loading && <p className={styles.subtitle}>{tag} follows {total} {total === 1 ? 'person' : 'people'}</p>}
      </header>
